...
const iso3Map = {
  ...
  "UK": "GBR",
  "Germany":"GER",
  "EU": "EU",            // no single polygon; skip or map to a specific EU country if you prefer
  "Italy": "ITA",
  "China": "CHN",
  "Australia": "AUS",
  "Canada": "CAN",
...
function highlightCountry(countryCode) {
    ...
    if (countryCode === "EU") {
        featuresToHighlight = countryGeoJson.features.filter(f =>
            euCountries.includes((f.properties.iso_a3 || f.properties.iso_a3).toUpperCase())
        );
    } else {
        const code = countryMap[countryCode] || countryCode;
        const feat = countryGeoJson.features.find(f =>
            (f.properties.iso_a3 || f.properties.iso_a3).toUpperCase() === code.toUpperCase()
        );
...
function updateCharts() {
    ...
    const countryMap = {
        ...
        "EU": "EU",
        "France":"FRA",
        "Germany": "DEU",          // you may change this to a different EU state if desired
        "Italy": "ITA",
        "China": "CHN",
        "Australia": "AUS",
        "Norway": "NOR",
